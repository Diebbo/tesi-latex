...
\section{Data Flow and Network Protocols}
\begin{itemize}
...
\begin{tabularx}{\linewidth}{|l|X|X|X|X|}  % Cambiato l l finale in X per migliorare l'allineamento
\hline
...
Parking Request & Proxy & MQTT Pub & Broker &
$\{proxy: p_i,\ positions: [\forall j \in \text{Parkings},\ \xi_{mc_i, park_j}(P_i)]\}$ \\ \hline
Parking Publish & LDS & POST & Broker & $\{\xi_{park_j}(P_j),\ \text{status} \in \{\text{free}, \text{occ.}\}\}$ \\ \hline
\end{tabularx}
