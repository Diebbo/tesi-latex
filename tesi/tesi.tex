...
\subsubsection{HTTPS}
HTTPS is a version of HTTP built on the SSL/TLS protocol, providing secure communication over a computer network. It encrypts data exchanged between clients and servers, ensuring confidentiality and integrity. HTTPS is essential for protecting sensitive information, such as login credentials and financial transactions, from eavesdropping and tampering.
...
By definition we can represent the GPS coordinates of a point \( (x_p, y_p) \) into a point \( p \). This point will be represented in the space of \( n \) orders \( \in \mathbb{Z}_4 \).
...
Given two Z-order encoded values \( Z_1 \) and \( Z_2 \), we can find their maximum common prefix by:
\begin{enumerate}
    \item Converting both values to their binary representation
    \item Comparing the bits from left to right
    \item Stopping at the first position where the bits differ
\end{enumerate}
...
\begin{align*}
    ...
    Z_2 &= 1110_2
\end{align*}
The maximum common prefix is \( 110_2 \), indicating that these points share the same region at the third level of the Z-order curve.
...
When implementing privacy-preserving location queries using Z-order encoding, we can employ homomorphic encryption to protect sensitive location data. Let's consider a scenario where a user \( A \) wants to share their location with user \( B \) while maintaining privacy:
...
To ensure unambiguous results, we increment each value in \( qk_i \in QK_A \) by one before encryption, such that \( qk_i \in \{1,2,3,4\} \). In this scheme:
\begin{itemize}
