...
Key Obtain & CA & HTTP GET & Backend & - \\ \hline
Key publish & MC & HTTP POST & CA & $\{id:ID_{mc}, pub. key: k_{mc}^+ \}$ \\ \hline
Position Publish & MC & MQTT Sub & Proxy & $\{position: \xi_{mc}( P_i )\}$ \\ \hline
Parking Request & Proxy & MQTT Pub & Backend & $\{proxy: pr_i,\ position: \xi_{mc_i}(P_i) \}\\ \hline
Key Request & LDS & HTTPS GET & CA & $id: ID_{park_j}$ \\ \hline
Parking Publish & LDS & POST & Backend & $\{\xi_{park_j}(P_j),\newline id:ID_{park_j}, \newline \text{status} \in \{\text{free}, \text{occ.}\}\}$ \\ \hline
