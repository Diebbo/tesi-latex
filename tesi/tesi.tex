...
\subsection{LDS}
The Location Data Source (LDS), commonly referred to as sensors, are responsible for collecting and publishing location data. In our system, we assume they indipendently publish their data to the broker, based on the event they register. For instance, if a sensor detects a free parking spot, it publishes the information to the broker, that will start the encryption protocol.
...
When the computation is finished, the broker publishes the results to the topic $ID_{MC_i}$, allowing the client to receive the updates on the parking spots. The client can then subscribe to the topic $ID_{MC_i}$ to receive updates on the parking spots.

\subsection{HTTP}
The HTTP protocol is used for all the traffic arriving to the backend server. The server is responsible for managin the storage of the encrypted parking spots data, for handling the requests from the proxy(s) and managing the workers.
...
\hline
\textbf{API} & \textbf{Subject} & \textbf{SubProtocol} & \textbf{Receiver} & \textbf{Payload} \\ \hline
Position Publish & MC & MQTT Sub & Proxy & $\{position: \xi_{mc}( P_i )\}$ \\ \hline
Key Obtain & MC & HTTP GET & Backend & - \\ \hline
Key publish & MC & HTTP POST & Proxy & $\{keys: [\forall j \in \text{Parkings},\ k_{mc_i, park_j}]\}$ \\ \hline
Parking Request & Proxy & MQTT Pub & Backend &
$\{proxy: p_i,\ positions: [\forall j \in \text{Parkings},\ \xi_{mc_i, park_j}(P_i)]\}$ \\ \hline
Parking Publish & LDS & POST & Backend & $\{\xi_{park_j}(P_j),\ \text{status} \in \{\text{free}, \text{occ.}\}\}$ \\ \hline
Work Request & Worker & HTTP POST & Backend & - \\ \hline
Task Finish & Worker & HTTP POST & Backend & $\{worker: w_i,\ task: t_j,\ result: \xi_{mc_i, park_j}(R)\}$ \\ \hline
Computation Resoult & Backend & MQTT Pub & Proxy & $\{proxy: p_i,\ resoult: [\forall j \in \text{Parkings},\ \xi_{mc_i, park_j}(R)]\}$ \\ \hline
...
\begin{itemize}
    \item mc encrypts its position using it's key and the public key of the parking
    \item the proxy receives with the publish event the blob of positions
    \item the proxy notify the broker with an HTTP POST requests. The body contains the position and translation key for each parking spot.
    \item the broker who stores the encrypted positions(with the parking private key) devides the working load among the workers. 
    \item When they receive the task, they make the encrypted operation. After they finish the send the resoult to the backend server.
    \item it the re-translate the resoult back using the translation key such that the client can receive that.
    \item the Proxy receives the resoult through the MQTT publish from the server.
    \item At this point the Proxy forward back the resoult to the client that subscribed to the topic.
